...

{
    public partial class ProgramTV : Plugin
    {
        ...
        private static readonly Regex programmeRegex = new Regex(
            "\\<div class=\\\"smartpe_progentry (?:smartpe_progentry_old)\\\".*?\\<time class=\\\"smartpe_time (?:smartpe_time_old)\\\".*?itemprop=\\\"startDate\\\"\\s?content=\\\"(.*?)\\\".*?\\<h3\\s?class=\\\"smartpe_progtitle_common smartpe_progtitle\\\"\\s?itemprop=\\\"(?:.*?)\\\"\\>\\<a\\s?id=\\\"(.*?)\\\"\\s?href=\\\"(.*?)\\\"\\s?target=\\\"(?:.*?)\\\"\\>(.*?)\\</a\\>\\</h3\\>.*?\\<div class=\\\"smartpe_progshortdesc\\\"(?: itemprop=\\\".*?\\\")?\\s?\\>(.*?)\\</div\\>.*?\\<div class=\\\"smartpe_progentrylong\\\"\\>(.*?)\\</div\\>", RegexOptions.Singleline | RegexOptions.Compiled);
